...
int __not_in_flash_func(InfoNES_HSync)()
{
  ...
  {
    // Called every frame. The platform hook flushes audio (the master clock)
    // unconditionally and only hands new video to the display core when
    // FrameCnt == 0 (a freshly rendered frame). Frame-skipped frames skip the
    // PPU pixel pipeline above but must still pace audio here.
    InfoNES_LoadFrame();
  }
