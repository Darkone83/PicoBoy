...
// ---- systems -------------------------------------------------------------
// Folder + extension per system. Only GB launches today (Peanut-GB is DMG);
// the rest are browsable now and get cores in phase 4.
// Folder + extension per system. ext2 is an optional second accepted extension.
// NULL ext2 = single extension.
typedef struct { const char *name; const char *dir; const char *ext; const char *ext2; bool runnable; } sys_t;
...
#define MQ_X0   30
#define MQ_X1   304
#define MQ_EVERY 2     // advance every N loop iters (~30 ms)
#define MQ_STEP  1     // px per advance
#define MQ_GAP   48    // px gap between wrap repeats
#define MQ_HOLD  20    // initial readable hold, in advance-ticks (~0.6 s)
#define MQ_MAXLEN 34   // names longer than this scroll instead of truncate
...
static int pick_list(const char *title, int count, const char *footer) {
    ...
    bool mq_on = false; int mq_y = 0, mq_off = 0, mq_hold = 0, mq_tick = 0;
    const char *mq_name = 0; uint16_t mq_fg = 0, mq_bg = 0;
    while (true) {
        if (redraw) {
            ...
            for (int r = 0; r < ROWS_VISIBLE && (top + r) < count; r++) {
                ...
                if (on) ui_fill_pill(8, y - 3, LCD_W - 22, ROW_H - 2, bg);
                ...
                if (on && longname) {
                    ...
                    st7789_marquee(MQ_X0, MQ_X1, y, s_names[i], 0, fg, bg, MQ_GAP);
                    mq_on = true; mq_y = y; mq_name = s_names[i];
                    mq_fg = fg; mq_bg = bg; mq_off = 0; mq_hold = MQ_HOLD; mq_tick = 0;
                } else {
                    char line[44];
                    if (longname)   // non-selected long: truncate with ellipsis
                        snprintf(line, sizeof line, "%c %.31s...", on ? '>' : ' ', s_names[i]);
                    else
                        snprintf(line, sizeof line, "%c %s", on ? '>' : ' ', s_names[i]);
                    st7789_draw_string(14, y, line, fg, bg, 1);
                }
            }
            // Scrollbar, only when the list overflows the window.
            if (count > ROWS_VISIBLE) {
                int tx = LCD_W - 6, ty = ROW_Y0 - 2, th = ROWS_VISIBLE * ROW_H;
                st7789_fill_rect(tx, ty, 4, th, g_theme->footer_bg);
                int thumb = th * ROWS_VISIBLE / count; if (thumb < 6) thumb = 6;
        ...
        if (mq_on && ++mq_tick >= MQ_EVERY) {
            mq_tick = 0;
            if (mq_hold > 0) mq_hold--;
            else             mq_off += MQ_STEP;
            st7789_marquee(MQ_X0, MQ_X1, mq_y, mq_name, mq_off, mq_fg, mq_bg, MQ_GAP);
        }
        buttons_update();
        ...
        if (ev & (1u << BTN_UP) && sel > 0) {
            sel--; if (sel < top) top = sel; redraw = true;
        }
        if (ev & (1u << BTN_DOWN) && sel < count - 1) {
            sel++; if (sel >= top + ROWS_VISIBLE) top = sel - ROWS_VISIBLE + 1; redraw = true;
        }
        // LEFT/RIGHT page by a full screen (only when the list overflows).
        if (count > ROWS_VISIBLE) {
            if (ev & (1u << BTN_RIGHT) && sel < count - 1) {          // page down
                sel += ROWS_VISIBLE; if (sel > count - 1) sel = count - 1;
                if (sel >= top + ROWS_VISIBLE) top = sel - ROWS_VISIBLE + 1;
                redraw = true;
            }
            if (ev & (1u << BTN_LEFT) && sel > 0) {                   // page up
                sel -= ROWS_VISIBLE; if (sel < 0) sel = 0;
                if (sel < top) top = sel;
                redraw = true;
            }
        }
...
// Remember the last launched game (sys + base name) so "Load last game" can map
// it back to its .srm (and, in 3c, save-state) paths after a reboot.
static void persist_last(const char *sysdir, const char *base) {
    FIL f;
    if (f_open(&f, "/lastrom.txt", FA_WRITE | FA_CREATE_ALWAYS) == FR_OK) {
        // "<sys>/<base>\n<flash size>" - size is recorded for forward
        // compatibility; the relaunch path no longer needs it.
...
static void ensure_dirs(void) {
    f_mkdir("/roms"); f_mkdir("/save"); f_mkdir("/state");   // FR_EXIST is fine
    char p[32];
    for (int i = 0; i < NSYS; i++) {
        snprintf(p, sizeof p, "/roms/%s",  SYS[i].dir); f_mkdir(p);
        snprintf(p, sizeof p, "/save/%s",  SYS[i].dir); f_mkdir(p);
        snprintf(p, sizeof p, "/state/%s", SYS[i].dir); f_mkdir(p);
    }
}
...
static int scan_dir(const char *path, const char *ext, const char *ext2) {
    ...
    int n = 0;
    while (n < MAX_ENTRIES && f_readdir(&dir, &fno) == FR_OK && fno.fname[0]) {
        if (fno.fattrib & AM_DIR) continue;
        if (!has_ext(fno.fname, ext) && !(ext2 && has_ext(fno.fname, ext2))) continue;
        strncpy(s_names[n], fno.fname, MAX_NAME - 1);
        s_names[n][MAX_NAME - 1] = '\0';
        n++;
    }
    f_closedir(&dir);
    if (n > 1) qsort(s_names, n, MAX_NAME, name_cmp);   // alphabetical (case-insensitive)
    return n;
}
...
static bool flash_from_sd(const char *path) {
    FIL fil;
    if (f_open(&fil, path, FA_READ) != FR_OK) { show_msg("ROM Loader", "Open failed", COL_RED); return false; }

    uint32_t size   = (uint32_t)f_size(&fil);
    uint32_t window = NVS_FLASH_OFFSET - ROM_FLASH_OFFSET;
    if (size == 0 || size > window) {
        f_close(&fil);
        show_msg("ROM Loader", size == 0 ? "Empty file" : "ROM too big for flash", COL_RED);
        return false;
    }
    s_last_flash_size = size;                 // recorded into /lastrom.txt
    ...
    for (uint32_t off = 0; off < size; off += FLASH_SECTOR_SIZE) {
        UINT br = 0;
        if (f_read(&fil, s_secbuf, FLASH_SECTOR_SIZE, &br) != FR_OK) { ok = false; break; }
        if (br < FLASH_SECTOR_SIZE) memset(s_secbuf + br, 0xFF, FLASH_SECTOR_SIZE - br);
    ...
    if (!ok) { show_msg("ROM Loader", "SD read error", COL_RED); return false; }
    return true;
}
...
void loader_browse(void) {
    status("Mounting SD...");
    if (f_mount(&s_fs, "", 1) != FR_OK) {        // opt=1: mount now (forces card init)
    ...
    while (true) {
        ...
        if (flash_from_sd(full)) {
            ...
            persist_last(SYS[s].dir, base);   // so Load-last maps to the same files
            led_set_state(LED_RUNNING);
            if (strcmp(SYS[s].dir, "nes") == 0) {
                char srm[96];
                snprintf(srm, sizeof srm, "/save/%s/%s.srm", SYS[s].dir, base);
                nes_set_save_path(srm);       // battery SRAM saves
                nes_run();
            } else {                          // gb
                char srm[96], dat[96];
                snprintf(srm, sizeof srm, "/save/%s/%s.srm",  SYS[s].dir, base);
                snprintf(dat, sizeof dat, "/state/%s/%s.dat", SYS[s].dir, base);
                gb_set_save_path(srm);
                gb_set_state_path(dat);
                gb_core_run();                // reads the ROM from the flash window (XIP)
            }
        }
...
// "Load last game": run whatever is staged in the flash window. If the card is
// present and /lastrom.txt resolves, wire up that game's .srm so saves continue;
// otherwise just run the window (instant relaunch, no card needed).
void loader_launch_last(void) {
    int sys_kind = 0;                             // 0 = gb, 1 = nes
    gb_set_save_path("");                          // defaults: no save/state unless resolved
    gb_set_state_path("");
    nes_set_save_path("");
    if (f_mount(&s_fs, "", 1) == FR_OK) {
        ...
        if (f_open(&f, "/lastrom.txt", FA_READ) == FR_OK) {
            ...
            f_close(&f);
            line[br] = '\0';
            // File is "<sys>/<base>" optionally followed by "\n<flash size>".
            // Split the optional size line off so <base> never absorbs it.
            char *nl = strchr(line, '\n');
            if (nl) *nl = '\0';
            // trim trailing CR/space from the name line
            int nlen = 0; while (line[nlen]) nlen++;
            while (nlen && (line[nlen - 1] == '\r' || line[nlen - 1] == ' '))
                line[--nlen] = '\0';
            ...
            if (slash) {
                *slash = '\0';
                ...
                char srm[96], dat[96];
                snprintf(srm, sizeof srm, "/save/%s/%s.srm",  sysdir, base);
                snprintf(dat, sizeof dat, "/state/%s/%s.dat", sysdir, base);
                gb_set_save_path(srm);
                gb_set_state_path(dat);
                nes_set_save_path(srm);   // harmless for GB; used when the last game is NES
                if      (strcmp(sysdir, "nes")  == 0) sys_kind = 1;
            }
        }
    }
    led_set_state(LED_RUNNING);
    if      (sys_kind == 1) nes_run();
    else                    gb_core_run();
}