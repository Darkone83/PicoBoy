...
/* PicoBoy / RP2040: NSF (.nsf music) playback is never reached here -- the
 * loader exposes no NSF entry, so these hooks are only ever wired when an
 * NSF image is loaded, which cannot happen on this build. Kept in flash
 * rather than spending scarce SRAM on code that never runs. */
...
static void NsfApplyBank(int reg)
{
