...
// ---- State thresholds -------------------------------------------------------
#define FULL_PCT               95      // SoC at/above this (and not rising) -> full
#define LOW_ENTER_PCT          15      // assert LOW at/under this
#define LOW_EXIT_PCT           22      // clear LOW above this (hysteresis)
...
// long and the decision debounced to keep it stable. *** For deterministic charge/
// full, wire TP4056 CHRG# (and STDBY#) to a spare GPIO and define CHRG_PIN/STDBY_PIN
// in pins.h -- the code below uses the pins automatically when they're defined. ***
...
static int      s_pct=0;
static bool     s_full=false, s_charging=false, s_low=false;
...
static bool poll(repeating_timer_t *t) {
    ...
    if (!valid()) {
        s_ema_init=false; s_pct=0; s_full=s_charging=s_low=false; s_chg_cnt=0;
...
#if defined(CHRG_PIN)
    (void)slope;
    charging = (gpio_get(CHRG_PIN) == 0);              // deterministic (open-drain, low = charging)
  #if defined(STDBY_PIN)
    full = (gpio_get(STDBY_PIN) == 0);                 // low = charge complete
  #else
    full = !charging && (s_pct >= FULL_PCT);
  #endif
#else
    // Voltage-only: rising = charging (debounced), high+flat = full.
    ...
    led_batt_t b = s_charging ? LED_BATT_CHARGING
                 : s_full     ? LED_BATT_FULL
                 : s_low      ? LED_BATT_LOW
                 :              LED_BATT_NONE;
    led_set_battery(b);
    ...
    printf("[batt] vbat=%lumV ema=%lumV pct=%d slope=%+dmV/%ds %s\n",
           ...
           s_charging?"CHARGING":s_full?"FULL":s_low?"LOW":"-");
#endif
    return true;
}
...
#if defined(CHRG_PIN)
    gpio_init(CHRG_PIN);  gpio_set_dir(CHRG_PIN,  GPIO_IN);   // external 10k pull-up
#endif
#if defined(STDBY_PIN)
    gpio_init(STDBY_PIN); gpio_set_dir(STDBY_PIN, GPIO_IN);
#endif
    poll(NULL);
