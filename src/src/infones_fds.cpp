...
/* PicoBoy / RP2040: FDS (Famicom Disk System) is a parked RP2350 feature and
 * the loader exposes no FDS entry; fdsRenderAudio is gated behind ApuFdsEnable
 * (false for every cartridge game). This whole audio chain is dead code on
 * RP2040, so keep it in flash instead of burning SRAM. */
...
/*-------------------------------------------------------------------*/
static inline bool fdsTickEnvelope(BYTE speed, BYTE env_off,
...
/*-------------------------------------------------------------------*/
static void fdsUpdateModOutput(WORD volumePitch)
{
...
/*-------------------------------------------------------------------*/
static inline void fdsUpdateModCounter(int8_t value)
{
...
static inline bool fdsTickModulator()
{
